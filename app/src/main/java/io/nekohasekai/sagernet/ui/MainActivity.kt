...
class MainActivity : ThemedActivity(),
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        binding.stats.visibility = android.view.View.INVISIBLE
    ...
    override fun onResume() {
        ...
        if (currentFragment is ConfigurationFragment) {
            binding.stats.allowShow = true
            binding.fab.show()
            // 只有在主界面且连接状态时才显示StatsBar
    ...
    fun displayFragment(fragment: ToolbarFragment) {
        // 优化界面切换时的UI体验：只有主界面显示连接按钮和状态栏
        if (fragment is ConfigurationFragment) {
            binding.stats.allowShow = true
            binding.fab.show()
            // 在主界面时，根据连接状态显示或隐藏StatsBar
            if (DataStore.serviceState == BaseService.State.Connected) {
                binding.stats.visibility = android.view.View.VISIBLE
                binding.stats.performShow()
            } else {
                binding.stats.visibility = android.view.View.INVISIBLE
            }
        } else {
            // 对于所有非主界面，立即隐藏连接按钮和状态栏
            binding.stats.allowShow = false
            binding.stats.visibility = android.view.View.INVISIBLE
            binding.stats.performHide()
            binding.fab.hide()
